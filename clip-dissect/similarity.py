...
def cos_similarity_gt(clip_feats, target_feats, device='cuda', gt_labels=None, num_classes=10):
    with torch.no_grad():
        clip_feats = clip_feats / torch.norm(clip_feats, p=2, dim=0, keepdim=True)
        target_feats = target_feats / torch.norm(target_feats, p=2, dim=0, keepdim=True)
        
        batch_size = 10000
        # print(target_feats.shape, clip_feats.shape, gt_labels.shape)
        gt_labels = F.one_hot(gt_labels, num_classes)
        clip_feats = clip_feats * gt_labels
...
def cos_similarity_cubed_gt(clip_feats, target_feats, device='cuda', batch_size=10000, min_norm=1e-3, gt_labels=None, num_classes=10):
    """
    Substract mean from each vector, then raises to third power and compares cos similarity
    Does not modify any tensors in place
    """
    with torch.no_grad():
        ...
        # masking clip features based on GT labels to avoid using irrelevant class images
        gt_labels = F.one_hot(gt_labels, num_classes)
        clip_feats = clip_feats * gt_labels
...
# this does not use clip_feats, just keeping clip_feats to make it work in same codebase
def majority_vote_gt(clip_feats, target_feats, top_k=100, device='cuda',
                        gt_labels=None, num_classes=10):
    ...
    with torch.no_grad():
        ...
        for orig_id in tqdm(range(target_feats.shape[1])):
            curr_gt_labels = gt_labels.gather(0, inds[:,orig_id:orig_id+1].squeeze(1)).to(device)

            curr_vote_count = torch.bincount(curr_gt_labels, minlength=num_classes)

            vote_counts.append(curr_vote_count.reshape(1, -1))
            torch.cuda.empty_cache()

        vote_counts = torch.cat(vote_counts, dim=0)
        print(vote_counts.shape)

    return vote_counts


# modified soft wpmi to use GT information 
# increased topk from 100 to 1000 so that more images are used (and filtered by GT label)
# maybe in 100, not enough samples from GT in some cases, so I increased it to 1000
# Example: gt_labels is a list of 50000 integer labels (0 to 9) for CIFAR10
def soft_wpmi_gt(clip_feats, target_feats, top_k=50000, a=10, lam=1, device='cuda',
                        min_prob=1e-7, p_start=0.998, p_end=0.97, gt_labels=None, num_classes=10):
    
    with torch.no_grad():
        torch.cuda.empty_cache()
        clip_feats = torch.nn.functional.softmax(a*clip_feats, dim=1)

        # masking clip softmax features based on GT labels to avoid using irrelevant class images
        gt_labels = F.one_hot(gt_labels, num_classes)
        clip_feats = clip_feats * gt_labels
        ...
        # instead of ranking based probability (as per Appendix A.1 of CLIP Dissect paper),
        # we will use the GT labels to get 0 or 1 as p(x_i \in B_k)
        p_in_examples = p_start-(torch.arange(start=0, end=top_k)/top_k*(p_start-p_end)).unsqueeze(1).to(device)
        for orig_id in tqdm(range(target_feats.shape[1])):
            ...
            curr_p_d_given_e = 1+p_in_examples*(curr_clip_feats-1)
            # print(curr_p_d_given_e.shape, p_in_examples.shape, curr_clip_feats.shape)
            curr_p_d_given_e = torch.sum(torch.log(curr_p_d_given_e+min_prob), dim=0, keepdim=True)
            prob_d_given_e.append(curr_p_d_given_e)
            torch.cuda.empty_cache()
